...
read_ini_to_execute_command(){
	...
	# echo "set_key_value ${set_key_value}"
	...
	# echo "exec_key_value_con: ${exec_key_value_con}"
	...
	EXEC_BEFORE_COMMAND="$(echo "${variabl_contensts_setting_value_list[6]}" | sed 's/^"//' | sed 's/"$//')"
	EXEC_AFTER_COMMAND="$(echo "${variabl_contensts_setting_value_list[7]}" | sed 's/^"//' | sed 's/"$//')"
}
