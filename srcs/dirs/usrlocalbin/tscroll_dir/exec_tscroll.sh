...
DIR_NAME=$(dirname $0)
TOUCHPAD_DEVICE_NAME=$(xinput --list --name-only | grep -i  -e touchpad -e glidepoint -e trackpad)
...
scroll_setting_dialog () {
  ...
  CURRENT_SCROLL_DISTANCE=$(xinput list-props "${TOUCHPAD_DEVICE_NAME}" | grep "${SETTING_CLUMUN}" | awk '{print $6}')
...
keybord_disable_touchpad_dialog () {
  while :
  do
	CURRENT_TYPE_DISABLE_TIME=$(ps -ef | grep syndaemon | head -n 1 |  awk '{print $10}')
  echo "###############################################################"
